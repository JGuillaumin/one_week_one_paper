...
def _bn_act(x, bn_name=None, p=0.95):
    """Helper to build a BN -> relu block
    """
    norm = BatchNormalization(axis=CHANNEL_AXIS, name=bn_name)(x)
    return DropActivation(p=p)(norm)


def _conv_bn_act(**conv_params):
    ...
    p = conv_params.setdefault("p", 0.95)
    ...
    def f(x):
        x = Conv2D(filters=filters, kernel_size=kernel_size,
                   ...
                   name=conv_name)(x)
        return _bn_act(x, bn_name=bn_name, p=p)

    return f


def _bn_act_conv(**conv_params):
    ...
    def f(x):
        activation = _bn_act(x, bn_name=bn_name, p=p)
...
def bottleneck(filters, stage, block, transition_strides=(1, 1),
               dilation_rate=(1, 1), is_first_block_of_first_layer=False, dropout=None,
               residual_unit=_bn_act_conv, p=0.95):
...
def ResNet(input_shape=None, classes=10, block='bottleneck', residual_unit='v2', repetitions=None,
           ...
           initial_pooling='max', final_pooling=None, top='classification', p=0.95):
    ...
    x = _conv_bn_act(filters=initial_filters, kernel_size=initial_kernel_size, strides=initial_strides, p=p)(img_input)
...
def ResNet18(input_shape, classes, p):
    """ResNet with 18 layers and v2 residual units
    """
    return ResNet(input_shape, classes, basic_block, repetitions=[2, 2, 2, 2], p=p)


def ResNet34(input_shape, classes, p):
    """ResNet with 34 layers and v2 residual units
    """
    return ResNet(input_shape, classes, basic_block, repetitions=[3, 4, 6, 3], p=p)


def ResNet50(input_shape, classes, p):
    """ResNet with 50 layers and v2 residual units
    """
    return ResNet(input_shape, classes, bottleneck, repetitions=[3, 4, 6, 3], p=p)


def ResNet101(input_shape, classes, p):
    """ResNet with 101 layers and v2 residual units
    """
    return ResNet(input_shape, classes, bottleneck, repetitions=[3, 4, 23, 3], p=p)


def ResNet152(input_shape, classes, p):
    """ResNet with 152 layers and v2 residual units
    """
    return ResNet(input_shape, classes, bottleneck, repetitions=[3, 8, 36, 3], p=p)